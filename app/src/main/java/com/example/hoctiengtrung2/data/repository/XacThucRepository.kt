...
class XacThucRepository(private val remoteDataSource: XacThucRemoteDataSource) {
    ...
    suspend fun capNhatThongTin(idNguoiDung: String, ten: String, target: Int) =
        remoteDataSource.capNhatThongTin(idNguoiDung, ten, target)

    suspend fun doiMatKhau(idTK: String, matKhauMoi: String) =
        remoteDataSource.doiMatKhau(idTK, matKhauMoi)
}