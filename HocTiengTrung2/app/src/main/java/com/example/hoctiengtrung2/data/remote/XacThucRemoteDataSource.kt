...
class XacThucRemoteDataSource {
    ...
    suspend fun register(taiKhoan: TaiKhoan): NguoiDung? {
        return try {
            ...
            val newNguoiDung = NguoiDung(
                ...
                chuoiNgay = 0,              // Khởi tạo chuỗi ngày học bằng 0 cho tài khoản mới
                ngayDaHoc = emptyList()     // Khởi tạo mảng ngày đã học trống rỗng ban đầu
            )
    ...
    suspend fun capNhatThongTin(idNguoiDung: String, ten: String, tuoi: Int, target: Int): Boolean {
        return try {
